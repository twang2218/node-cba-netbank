...
describe('parser.js', () => {
  describe('- parseTitle()', () => {
    ...
    test('should parse home page title to "Home"', () => {
      ...
      return expect(parser
        .parseTitle({
          url: links.home,
          headers: headersHtmlWithCharset,
          body: pages.home,
        })
        .then((resp) => {
  ...
  describe('- serializeArray()', () => {
    ...
    test('should get serialized form from HTML page (history page)', () => {
      const form = {};
      parser.serializeArray(cheerio.load(pages.history)('form'), { disabled: true, button: true }).forEach((item) => {
        form[item.name] = item.value;
      });
  ...
  describe('- parseForm()', () => {
    ...
    test('should be able parse the properties', () => {
      ...
      return expect(parseForm(pages.logon)
        .then((resp) => {
    ...
    test('should parse <input type="radio" ...>', () => {
      expect.assertions(4);
      return expect(parseForm(pages.history)
        .then((resp) => {
    ...
    test('should parse <select><option ...>...</select>', () => {
      ...
      return expect(parseForm(pages.history)
        .then((resp) => {
          expect(resp.form.ctl00$ContentHeaderPlaceHolder$ddlAccount$field).toEqual('5218012345678901,MCD,True,True,True,False,True,False,True,False');
  ...
  describe('- parseAccountList()', () => {
    test('should parse account list', () => {
      ...
      return expect(parser
        ...
        .then((resp) => {
          ...
          expect(resp.accounts[1].balance).toEqual(1234.5);
          expect(resp.accounts[1].available).toEqual(234.5);
          ...
          expect(resp.accounts[2].balance).toEqual(76543.0);
          expect(resp.accounts[2].available).toEqual(76043.0);
    ...
    test('should raise error if there is no account list in the page', () => {
      expect.assertions(1);
      return expect(parser.parseAccountList({
        url: links.login,
        headers: headersHtml,
        body: pages.logon,
      })).rejects.toBeDefined();
  ...
  describe('- parseHomePage()', () => {
    ...
    test('should raise error if fail to parse the account list in the home page.', () => {
      expect.assertions(1);
      return expect(parser.parseHomePage({
        url: links.login,
        headers: headersHtml,
        body: pages.logon,
      })).rejects.toBeDefined();
  ...
  describe('- parseCurrencyHtml()', () => {
    ...
    cases.forEach((c) => {
      test(`should parse "${c.currency} ${c.post}" to ${c.number}`, () => {
        expect(parser.parseCurrencyHtml(`<div class="FieldElement FieldElementLabel FieldElementNoLabel">
  ...
  describe('- parseTransactionPage()', () => {
    ...
    test("should raise error if it's a page without form.", () => {
      expect.assertions(1);
      return expect(parser.parseTransactionPage({
        url: links.history,
        headers: headersJson,
        body: pages.historyPartial,
      })).rejects.toBeDefined();
